...
from ui.utils import (
    _read_csv_robust,
    df_to_markdown_row,
    discover_with_reasons,
    display_name_for,
    get_registry,
    list_personas,
    load_normalization_rules,
    normalize_calendar,
    save_normalization_rules,
    upsert_registry,
)
                ...
                index=cols.index(
                    next(
                        (
                            c
                            for c in cols
                            if c.lower() in ["date", "day", "publish date", "post date", "(none)"]
                        ),
                        "(none)",
                    )
                ),
                ...
                index=cols.index(
                    next(
                        (
                            c
                            for c in cols
                            if c.lower() in ["topic", "hook theme", "title", "theme", "(none)"]
                        ),
                        "(none)",
                    )
                ),
                ...
                index=cols.index(
                    next(
                        (
                            ...
                            or c.lower() in ["ardent service tie-in", "(none)"]
                        ),
                        "(none)",
                    )
                ),
                ...
                index=cols.index(
                    next(
                        (
                            c
                            for c in cols
                            if "pillar" in c.lower() or c.lower() in ["content pillar", "(none)"]
                        ),
                        "(none)",
                    )
                ),
...
if valid:
    ...
    with st.sidebar.expander("Calendar name (alias)"):
        ...
        new_alias = st.text_input(
            "Display name", value=current_alias, help="Alias for this calendar"
        )
    ...
    if sel_file:
        try:
            ...
            if st.button("Download normalized CSV"):
                buf = io.StringIO()
                out_df = df.copy()
                if "date" in out_df.columns and pd.api.types.is_datetime64_any_dtype(
                    out_df["date"]
                ):
                ...
                st.download_button(
                    "Save file",
                    data=buf.getvalue(),
                    file_name=f"normalized_{sel_file}",
                    mime="text/csv",
                )
...
with col_right:
    ...
    if controls.get("run") and sel:
        ...
        st.download_button(
            "Download CSV", data=csv_buf.getvalue(), file_name="linkedin_post.csv", mime="text/csv"
        )
